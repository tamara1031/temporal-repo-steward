...
func (s *refactorStepSuite) Test_Completed() {
	...
	env.OnActivity(acts.ReviewActivity, mock.Anything, mock.Anything).
		Return(codexact.ReviewResult{Verdict: codexact.ReviewVerdictOK}, nil)
	...
	s.Equal(workflow.StepOutcomeCompleted, result.Kind)
...
func (s *refactorStepSuite) Test_BudgetHalted_AdvisorRetries() {
	...
	env.OnActivity(acts.ReviewActivity, mock.Anything, mock.Anything).
		Return(codexact.ReviewResult{Verdict: codexact.ReviewVerdictCriticalBlock, Feedback: "not safe"}, nil)
	...
	env.OnActivity(acts.ConsultAdvisorActivity, mock.Anything, mock.Anything).
		Return(codexact.AdvisorVerdict{Verdict: codexact.AdvisorKindRetry, Rationale: "maybe next time"}, nil)
	...
	var result workflow.RefactorStepResult
	s.NoError(env.GetWorkflowResult(&result))
	s.Equal(workflow.StepOutcomeBudgetHalted, result.Kind)
}
...
func (s *refactorStepSuite) Test_CircuitBroken_AdvisorAborts() {
	...
	env.OnActivity(acts.ConsultAdvisorActivity, mock.Anything, mock.Anything).
		Return(codexact.AdvisorVerdict{Verdict: codexact.AdvisorKindAbort, Rationale: "too risky"}, nil)
...
func (s *refactorStepSuite) Test_Retries_WhenFirstIterationBlocked() {
	...
	env.OnActivity(acts.ReviewActivity, mock.Anything, mock.Anything).
		Return(codexact.ReviewResult{Verdict: codexact.ReviewVerdictCriticalBlock, Feedback: "fix it"}, nil).Once()
