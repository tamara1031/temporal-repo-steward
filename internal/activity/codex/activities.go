...
// ParsePlan extracts a Plan from a raw codex response. Returns the plan and
// true only when it contains at least a theme or one step.
func ParsePlan(raw string) (Plan, bool) {
	var p Plan
	if err := extractJSON(raw, &p); err != nil {
		return Plan{}, false
	}
	return p, p.Theme != "" || len(p.Steps) > 0
}
