...
describe('pollPostMergeOutcome lifecycle waits', () => {
  ...
  it.each([0, -10])('normalizes %s ms post-merge poll intervals before sleeping', async (pollIntervalMs) => {
  ...
  it('returns merge-queued without sleeping when the activity wait budget is exhausted', async () => {
    const poll = makePostMergePoll(['OPEN']);
